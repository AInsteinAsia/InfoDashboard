...
4. 涉及字符串列的 JOIN 或比较，必须加 `COLLATE Chinese_PRC_CI_AS` 避免排序规则冲突，例如：`a.col = b.col COLLATE Chinese_PRC_CI_AS`
5. 使用 `@st.cache_data(ttl=180)` 缓存所有查询函数（3 分钟）
6. 使用 `plotly.express` 绘图，`st.plotly_chart(fig, use_container_width=True)`
7. 筛选控件全部放在 `st.sidebar`
8. 用 `st.error()` 捕获并展示数据库异常，不要让 app crash
9. 第一行：`st.set_page_config(layout="wide", page_title="<看板标题>")`
10. 在 `st.set_page_config` 之后立即加自动刷新：`from streamlit_autorefresh import st_autorefresh` / `st_autorefresh(interval=180_000, key="autorefresh")`
